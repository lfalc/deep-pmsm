...
class CNNKerasRegressor(KerasRegressor):
    ...
    def fit_generator(self, seq, **kwargs):
        ...
        # fit_args = copy.deepcopy(Sequential.fit_generator)
        # fit_args.update(kwargs)

        fit_args = kwargs
        del fit_args["x_shape"]
    ...
    def score(self, x, y, **kwargs):
        ...
        if kwargs.pop("score_directly", False):
            ...
            x, sample_weights = self._generate_batches(
    ...
    def _generate_batches(
        _df_x, _df_y, p_id_col, batch_size, window_size, shuffle=True
    ):
                ...
                [
                    zero_df.assign(
                        **{
                            c: df[c].iloc[0]
                            for c in df.columns
                            if c.startswith(("ambient", "coolant"))
                            and not "_ew_rolling_std_" in c
                        }
                    ),
                    df,
                ],
        ...
        seq = LoadprofileGenerator(
            samples, target, length=window_size, shuffle=shuffle, batch_size=batch_size
        )
...
def build_cnn_model(
    x_shape=(256, 100),  # window_size x n_features
    arch="plain",
    n_layers=2,
    n_units=64,
    activation="relu",
    ...
    cnn_cfg = {
        ...
        "batchnorm": "_weightnorm" not in optimizer and activation != "selu",
    }

    model = cnn_network(**cnn_cfg)

    opt = opts_map[optimizer](lr_rate)
...
def cnn_network(
    ...
    dropout_layer = (
        SpatialAlphaDropout1D if activation == "selu" else layers.SpatialDropout1D
    )
    ...
    def residual_block(z, dilation_start_rate):
        ...
        z = layers.Conv1D(
            ...
            kernel_size,
            padding="causal",
            dilation_rate=dilation_start_rate,
            activation=None,
            input_shape=x_shape,
            **regs,
        )(z)
...
def get_predefined_model():
    ...
    y = layers.Conv1D(filters=126, kernel_size=2, padding="causal", name="conv1d_154")(
        x
    )
    # y = layers.BatchNormalization(name='batch_normalization_118')(y)
    y = layers.Activation("relu", name="activation_118")(y)
    y = layers.Conv1D(
        filters=126, kernel_size=2, padding="causal", dilation_rate=2, name="conv1d_155"
    )(y)
    # y = layers.BatchNormalization(name='batch_normalization_119')(y)
    y = layers.Activation("relu", name="activation_119")(y)

    shortcut = layers.Conv1D(
        filters=126, kernel_size=1, padding="causal", dilation_rate=2, name="conv1d_156"
    )(x)
